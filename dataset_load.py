...
class BasicDataLoader(object):
    ...
    def _prepare_data(self, config):
        ...
        for sample in tqdm(self.data):
            ...
            for i, tpl in enumerate(sample['subgraph']['tuples']):
                # if i % (100//config['completeness']): continue
                sbj, rel, obj = tpl
                try:
    ...
    def _build_fact_mat(self, sample_ids):
        """
        ...
        for i, sample_id in enumerate(sample_ids):           
            ...
            if self.use_self_loop:
                ...
                real_head_list = np.append(real_head_list, ent_array)
                real_tail_list = np.append(real_tail_list, ent_array)
                real_rel_list = np.append(real_rel_list, rel_array)
            
            batch_heads.append(real_head_list)
            batch_rels.append(real_rel_list)
            batch_tails.append(real_tail_list)

            if self.edge_extension:
                num_ent = len(self.global2local_entity_maps[sample_id])
                edge_mat = torch.zeros((num_ent, num_ent), dtype=int)
                score_mat = torch.zeros((num_ent, num_ent), dtype=torch.float)             
                for index in range(len(head_list)):
                    edge_mat[head_list[index]][tail_list[index]] = rel_list[index]
                    score_mat[head_list[index]][tail_list[index]] = 1
                num_ents.append(num_ent)
                edge_mats.append(edge_mat)
                score_mats.append(score_mat)

        return batch_heads, batch_rels, batch_tails, num_ents, edge_mats, score_mats
...
class SingleDataLoader(BasicDataLoader):
    ...
    def __init__(self, config, word2id, relation2id, entity2id, tokenize, data_type="train"):
        ...
        self.edge_extension = config['edge_extension']      
        if config['fake_entities']:
            i2e = dict()
            with open(config['data_folder'] + config['fake2id'], 'r') as file:
                i = 0
                for line in file:
                    i2e[i] = line.strip()
                    i += 1
            ...
            '''
            f2e = dict()
            with open(config['data_folder']+'fake_entities.txt', 'r') as file:
                for line in file:
                    fake, real = line.strip().split('\t', 1)
                    f2e[fake] = real

            id2entity = self.id2entity
            t ,f = 0, 0
            for i, fake in id2entity.items():
                if fake[1] != '.':
                    t += 1
                    continue
                if fake in f2e.keys():
                    id2entity[i] = f2e[fake]
                    t += 1
                else:
                    # id2entity[i] = '</s>'
                    id2entity[i] = ' '
            ...
            with open(config['data_folder']+'heyman_space.txt', 'w+') as file:
                for i, real in id2entity.items():
                    file.write(real+'\n')
            file.close()
            # '''
        self.id2entity[len(self.id2entity)] = ' '
    ...
    def get_batch(self, iteration, batch_size, fact_dropout, q_type=None, test=False):
        ...
        actual_entities = np.vectorize(self.id2entity.get)(self.candidate_entities[sample_ids])
        seed_dist = self.seed_distribution[sample_ids]
        q_input = self.deal_q_type(q_type)
        kb_adj_mats = self._build_fact_mat(sample_ids)
...
def load_data(config, tokenize):
    ...
    relation_texts_inv = None
    if config['relation_word_emb']:
